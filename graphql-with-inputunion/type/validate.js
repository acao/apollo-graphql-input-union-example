...
function validateDirectives(context) {
  ...
  directives.forEach(function (directive) {
    ...
    directive.args.forEach(function (arg) {
      ...
      if (!(0, _definition.isInputType)(arg.type) && !(0, _definition.isInputUnionType)(arg.type)) {
...
function validateTypes(context) {
  ...
  Object.keys(typeMap).forEach(function (typeName) {
    ...
    } else if ((0, _definition.isInputUnionType)(type)) {
      // Ensure Unions include valid member types.
...
function validateInputUnionMembers(context, inputUnion) {
  var memberTypes = inputUnion.getTypes();

  if (memberTypes.length === 0) {
    context.reportError('Input Union type ' + inputUnion.name + ' must define one or more member types.', inputUnion.astNode);
  }

  var includedTypeNames = Object.create(null);
  memberTypes.forEach(function (memberType) {
    if (includedTypeNames[memberType.name]) {
      context.reportError('Input Union type ' + inputUnion.name + ' can only include type ' + (memberType.name + ' once.'), getInputUnionMemberTypeNodes(inputUnion, memberType.name));
      return; // continue loop
    }
    includedTypeNames[memberType.name] = true;
    if (!(0, _definition.isInputObjectType)(memberType)) {
      context.reportError('Input Union type ' + inputUnion.name + ' can only include Input Object types, ' + ('it cannot include ' + String(memberType) + '.'), getInputUnionMemberTypeNodes(inputUnion, String(memberType)));
    }
  });
}
...
function validateInputFields(context, inputObj) {
  ...
  fieldNames.forEach(function (fieldName) {
    ...
    // Ensure the type is an input type
    if (!(0, _definition.isInputType)(field.type) && !(0, _definition.isInputUnionType)(field.type)) {
...
function getInputUnionMemberTypeNodes(inputUnion, typeName) {
  return inputUnion.astNode && inputUnion.astNode.types && inputUnion.astNode.types.filter(function (type) {
    return type.name.value === typeName;
  });
}
